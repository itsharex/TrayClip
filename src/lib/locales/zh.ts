export const zh = {
  ...
  helpWindowOpsItems: [
    ...
    ["关闭主窗口", "点击右上角关闭图标，默认隐藏到托盘继续运行，可在设置中改为直接退出或每次询问"],
  ] as [string, string][],
  helpClipboard: "剪贴记录",
  helpClipboardItems: [
    ["自动采集", "TrayClip 会自动记录剪贴板内容（文本、富文本、图片、文件路径）"],
    ["复制", "点击记录的「复制」按钮，或双击记录内容区域，内容会重新写入剪贴板"],
    ["右键菜单", "右键点击记录可快速复制；内容为 JSON 时支持格式化复制；内容含链接时可直接打开"],
    ["置顶", "点击「置顶」按钮，置顶记录不会被自动清理"],
    ["删除", "点击「删除」按钮，确认后永久删除该条记录"],
    ["搜索", "在搜索框输入关键词，支持按内容、来源应用、文件路径过滤"],
    ["图片预览", "图片记录会显示缩略图，文件路径中的图片也会显示预览"],
    ["内容截断", "超长文本会被截断并标记「内容已截断」"],
  ] as [string, string][],
  helpGroups: "分组管理",
  helpGroupsItems: [
  ...
  helpKeyboardItems: [
    ...
    "快捷面板打开时自动选中第一条记录，可直接按 Enter 复制",
  ],
  helpSettings: "设置说明",
  helpSettingsItems: [
    ...
    ["暂停采集", "临时停止记录剪贴板内容"],
    ["快捷粘贴", "仅 Windows：从快捷面板复制后自动粘贴到之前的应用窗口"],
    ["链接提示", "开启后，复制内容包含链接时会弹出提示窗口，可直接打开"],
    ["快捷面板呼出位置", "选择「屏幕中间」或「跟随鼠标」"],
    ["数据备份", "支持导出为 ZIP 备份文件，包含所有记录、分组和设置"],
    ["数据还原", "导入之前导出的 ZIP 备份文件，会替换所有现有数据并重启应用"],
    ["清空历史", "清空所有普通记录，置顶记录不受影响"],
    ["主题切换", "点击标题栏主题图标切换亮色/暗色主题"],
    ["语言切换", "点击标题栏语言图标选择界面语言"],
  ] as [string, string][],
